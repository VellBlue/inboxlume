...
def _lifecycle_prompt(raw: dict[str, Any], expected_kind: str) -> str:
    ...
    return (
        ...
        "JSON with exactly kind, state, utility, date_relation, condition, confidence, "
        "and reason_codes. Return the expected kind unchanged. State must be one of: "
        f"{', '.join(LIFECYCLE_STATES)}. Date relation must be one of: "
        f"{', '.join(DATE_RELATIONS)}. Utility must contain four booleans: "
        f"Condition must be one of: {', '.join(LIFECYCLE_CONDITIONS)}. "
        "operational, evidentiary, personal, security. These dimensions are independent: "
...
def _threat_prompt(raw: dict[str, Any]) -> str:
    ...
    return (
        ...
        "link_action, plausible_legitimate_context, confidence, and reason_codes. "
